...
// RetryForever to retry forever if action returns error.
func RetryForever(action ActionFunc, retryInterval time.Duration, onFailed FailFunc) {
	Retry(action, retryInterval, onFailed, -1)
}

// RetryWithResultForever to retry forever if action returns error, action also returns TResult.
...
// RetryWithStatusForever to retry forever if action returns error, action with T also returns TResult.
...
// RetryWithResult to retry again if action returns error, action also returns TResult.
...
// RetryWithStatus to retry again if action returns error, action with T also returns TResult.
